...
$publicVip1Name = "${prefix}-vip1"

$domain1Name = "${prefix}app1" 

$publicVip1 = Get-AzurePublicIpAddress `
    -Name $publicVip1Name `
    -ResourceGroupName $rgName
...
if (!(Test-AzureResource `
    ...
    $lbFeIp1ConfigName = "lb-feip1"

    $lbFeIp1Config = New-AzureLoadBalancerFrontendIpConfig `
        -Name $lbFeIp1ConfigName `
        -PublicIpAddress $publicVIP1
    ...
    $lbInboundNatRules += New-AzureLoadBalancerInboundNatRuleConfig `
        -Name $ruleName `
        -FrontendIpConfigurationId $lbFeIp1Config.Id `
        -Protocol Tcp `
    ...
    $lb = New-AzureLoadBalancer `
        ...
        -ResourceGroupName $rgName `
        -Location $location `
        -FrontendIpConfiguration $lbFeIpConfig, $lbFeIp1Config `
